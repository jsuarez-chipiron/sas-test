...
import getRecordData from "@salesforce/apex/FCS_IdentifyCustomerController.getRecordData";
...
export default class App extends LightningElement {
  ...
  async handleChangeCustomerButtonClick(event) {
    try {
      ...
      await fireEvent(
        this.currentRecordData.caseId + "_customerChanged",
        event
      );
    } catch (error) {
  ...
  async connectedCallback() {
    try {
      ...
      await registerListener(
        this.currentRecordData.caseId + "_customerIdentified",
        this.handleCustomerIdentified,
        this
      );
      await registerListener(
        this.currentRecordData.caseId + "_customerChanged",
  ...
  async evaluateRecordOnOpen() {
    console.log(
      "Evaluating on open: " + JSON.stringify(this.currentRecordData)
    );
    if (this.currentRecordData.accountId) {
      ...
      if (
        Date.now() -
          Date.parse(this.currentRecordData.lastRetrievedFromSource) >
        3600000
      ) {
    ...
    try {
      ...
      if (account) {
        ...
        const recordInput = {
          ...
          tpAccountNumber: account.FCS_TPAccountNumber__c,
          codsId: account.FCS_FrequentFlyer__c,
