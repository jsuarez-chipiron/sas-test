...
CLAIMS_QUERY = "Select Case__c, Liability_PNR__c, Country__c, Customer_Claim_Category__c, Customer_Claim_Type__c, Customer_Reason_for_EU261_Claim__c, Delay_Length__c, Flight_Date__c, Flight_Number__c, Id, Name, Contact_Last_Name__c FROM LIA_Claim__c WHERE Case__c IN " # caseIds
...
PASSENGERS_QUERY = "Select Booking__c, Id, Identifier__c, Last_Name__c FROM Passenger__c WHERE Booking__c IN " # bookingIds
...
List<LIA_Claim__c> claims = [SELECT Id, Name FROM LIA_Claim__c WHERE Name IN :caseIds];
...
for (Case c : cases){
    ...
    CaseToBooking__c ctb = new CaseToBooking__c(Booking__c = oldBookingIdToNewBookingId.get(c.SuppliedCompany), Case__c = c.Id);
...
for (Segment__c s : Segments){
    ...
    toDelete.add(s);
    toInsert.add(newSegment);
}

// toUpdate.addAll(ContentDocumentLinks);
toInsert.addAll(CaseToBookings);
update toUpdate;
insert toInsert;
delete toDelete;
"""
...
def main():
    metaInfo = MetaInfo()
    objectNames = ['Case', 'LIA_Claim__c', 'CaseToBooking__c', 'ContentDocumentLink', 'Booking__c', 'Passenger__c', 'Segment__c', 'Flight__c', 'FlightIrregularities__c'] # TODO Airports
    objectToHandleFunction = {'Case' : handleCases, 'LIA_Claim__c': handleClaims, 'Booking__c': handleBookings, 'Passenger__c': handlePassengers, 'CaseToBooking__c': handleCaseToBookings, 'Flight__c': handleFlights, 'FlightIrregularities__c': handleFlightIrregularities, 'Segment__c': handleSegments, 'ContentDocumentLink': handleContentDocumentLinks}
    objectQueries = {'Case': CASE_QUERY, 'LIA_Claim__c': CLAIMS_QUERY, 'Booking__c': BOOKINGS_QUERY, 'Passenger__c': PASSENGERS_QUERY, 'CaseToBooking__c': CASETOBOOKINGS_QUERY, 'Flight__c': FLIGHTS_QUERY, 'FlightIrregularities__c': FLIGHTIRREGULARITIES_QUERY, 'Segment__c': SEGMENTS_QUERY, 'ContentDocumentLink': CONTENTDOCUMENTLINKS_QUERY}
    ...
    print('WARNING: Maxiumum amount of case records recommended: ~300 (Salesforce CPU-time etc).')
    ...
    for objectName in objectNames:
        ...
        elif objectName == 'ContentDocumentLink':
...
def createCSVFiles(metaInfo, taskCount):
    ...
    promptRunScript(3, CLEANUP_SCRIPT)
...
def writeToCsv(df, name):
    ...
    if name == 'LIA_Claim__c':
        ...
        df['CONTACT_LAST_NAME__C'] = df['CONTACT_LAST_NAME__C'].apply(maskLastName)
         # Create fake PNRs (GDPR)
    if name == 'Booking__c':
...
def maskLastName(x):
    existingMask = lastNameToMask.get(x)
    if (existingMask != None):
        return existingMask
